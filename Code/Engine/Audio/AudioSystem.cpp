...
{	
	m_fmodSystem->set3DListenerAttributes( listenerIndex, new FMOD_VECTOR{ -listenerPosition.y, listenerPosition.z, -listenerPosition.x }, new FMOD_VECTOR{},
		new FMOD_VECTOR{ -listenerForward.y, listenerForward.z, -listenerForward.x }, new FMOD_VECTOR{ -listenerUp.y, listenerUp.z, -listenerUp.x } );
}
...
SoundPlaybackID AudioSystem::StartSoundAt( SoundID soundID, Vec3 const& soundPosition, bool isLooped, float volume, float balance, float speed, bool isPaused )
{
	...
	if (channelAssignedToSound)
	{
		...
		channelAssignedToSound->setPan( balance );
		channelAssignedToSound->setLoopCount( loopCount );
		FMOD_VECTOR pos = FMOD_VECTOR{ -soundPosition.y, soundPosition.z, -soundPosition.x };
		FMOD_VECTOR vel = FMOD_VECTOR{};
		channelAssignedToSound->set3DAttributes( &pos, &vel, 0 );
	}
...
{
	FMOD::Channel* channelAssignedToSound = (FMOD::Channel*)soundPlaybackID;
	channelAssignedToSound->set3DAttributes( new FMOD_VECTOR{ -soundPosition.y, soundPosition.z, -soundPosition.x }, new FMOD_VECTOR{}, 0 );
}
